...
def test_prediction_plot(mock_plt, binary_image, labeled_image):
    ...
    # Mock for 3 axes
    mock_axes_3 = [MagicMock() for _ in range(3)]
    # Mock for 4 axes
    mock_axes_4 = [MagicMock() for _ in range(4)]
    ...
    # Test without GT
    prediction_plot(binary_image, labeled_image, labeled_image, gt_image=None, show_plot=True)
    assert mock_plt.subplots.called
    assert mock_plt.show.called
    
    # Test with GT
    prediction_plot(binary_image, labeled_image, labeled_image, gt_image=labeled_image, show_plot=True)
    assert mock_plt.subplots.called
