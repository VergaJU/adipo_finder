...
class FeatureExtraction:
    ...
    @staticmethod
    def extract_shape_size_features(segmented_image: np.ndarray, distance_image: np.ndarray) -> pd.DataFrame:
        """
    ...
    @staticmethod
    def extract_distance_from_shore(segmented_image: np.ndarray, bin_tissue_image: np.ndarray, smoothing_sigma: float = 40) -> pd.DataFrame:
        """
        ...
        """
        from .utils import Preprocessing # Import here to avoid circular dependency if utils imports features
        
        features = []

        #removes object that are far from land 
        blurred_image = Preprocessing.apply_gaussian_filter(bin_tissue_image, sigma=smoothing_sigma)
    ...
    @staticmethod
    def compute_ring_features(segmented_image: np.ndarray, tissue_mask: np.ndarray, ring_size: int = 10) -> pd.DataFrame:
        """
    ...
    @classmethod
    def calculate_features(cls, prefiltered_seg_image: np.ndarray, distance_image: np.ndarray, bin_mask_image: np.ndarray) -> pd.DataFrame:
        """
        ...
        dfs_feat = cls.extract_distance_from_shore(prefiltered_seg_image, bin_mask_image)
        itmoto_feat = cls.compute_ring_features(prefiltered_seg_image, bin_mask_image, ring_size=10)
        
        # Merge the three feature dataframes on 'segment_id'
        if sz_feat.empty: # handle empty case
             return pd.DataFrame(columns=['segment_id', 'area', 'eccentricity', 'compactness', 'max_dist', 'distance_from_shore', 'frac_ring_other_objects', 'frac_ring_tissue'])
