...
class Segmentation:
    ...
    @staticmethod
    def find_local_maxima(image: np.ndarray, min_distance: int = 1) -> tuple[np.ndarray, np.ndarray]:
        """
        ...
        coords = feature.peak_local_max(distance, footprint=np.ones((3, 3)),labels=image, min_distance=min_distance)
    ...
    @staticmethod
    def size_shape_filter(seg_input_image: np.ndarray, distance_image: np.ndarray, min_dist: float, max_dist: float, min_area: int, max_area: int) -> np.ndarray:
        """
    ...
    @staticmethod
    def far_from_shore_filter(seg_input_image: np.ndarray, bin_tissue_image: np.ndarray, min_shore_intensity: int = 30) -> np.ndarray:
        """
    ...
    @staticmethod
    def in_the_middle_of_the_ocean_filter_iter(segmented_final: np.ndarray, tissue_mask: np.ndarray, ring_size: int = 10, min_tissue_overlap: float = 0.2) -> tuple[np.ndarray, list, list]:
        """
    ...
    @classmethod
    def in_the_middle_of_the_ocean_filter(cls, segmented_input: np.ndarray, tissue_mask: np.ndarray, ring_size: int = 10, min_tissue_overlap: float = 0.2) -> tuple[np.ndarray, list, list]:
        """
        ...
        kept_ids = [] # Initialize kept_ids to be safe, though loop should run at least once logic might vary
        
        while (keep_going):
            filtered, rem_ids, kept_ids = cls.in_the_middle_of_the_ocean_filter_iter(filtered, tissue_mask, ring_size=ring_size, min_tissue_overlap=min_tissue_overlap)
            if rem_ids:
    ...
    @staticmethod
    def remove_jagged_spindle_objects(segmented_image: np.ndarray, ecc_thresh: float = 0.8, compactness_thresh: float = 0.5) -> tuple[np.ndarray, list]:
        """
    ...
    @classmethod
    def preprocess_input_image(cls, seg_input_img: np.ndarray, min_distance_seg_init: int = 30, min_island_area: int = 0) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        distance, markers = cls.find_local_maxima(inverted_opened_img, min_distance=min_distance_seg_init)
    ...
    @classmethod
    def prefilter_image(cls, preprocessed_seg_image: np.ndarray, distance_image: np.ndarray) -> np.ndarray:
        '''
        Removes really large and really small objects
        '''
        bef_filtering = preprocessed_seg_image
        prefiltered = cls.size_shape_filter(bef_filtering, distance_image, min_dist=5, max_dist=200, min_area=30, max_area=12000)
        return prefiltered
