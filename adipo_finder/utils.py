...
class Preprocessing:
    @staticmethod
    def extract_segmentation_image(
        adata: AnnData,
        library_id: str,
    ...
    @staticmethod
    def segmentation_to_binary(
        segmentation: np.ndarray, threshold: float = 0.5, **kwargs
    ) -> np.ndarray:
        """
    ...
    def apply_gaussian_filter(
        image: np.ndarray, sigma: float = 1.0, **kwargs
    ) -> np.ndarray:
        """
        ...
        if sigma is not None:
            image = filters.gaussian(image, sigma=sigma, preserve_range=True).astype(
                "uint8"
            )
        return image
...
class Exporting:
    ...
    def expand_df(
        ...
        **kwarg,
        ...
        missing_data = (
            adata.obs[adata.obs[library_key] == library_id][missing_columns]
            .iloc[0]
            .to_dict()
        )
        df.index = (
            "s" + library_id.split("_")[1] + "_" + df[cell_id].astype(int).astype(str)
        )
    ...
    @staticmethod
    def create_adipo_adata(
        adata: AnnData, df: pd.DataFrame, spatial_key: str = "spatial", **kwargs
    ) -> AnnData:
        """
    ...
    def merge_adatas(
        ...
        **kwargs,
        ...
        adata_new.uns[spatial_key][library_id][image_key][
            segmentation_key
        ] = new_segmentation
        return adata_new
...
def process_ground_truth_image(base_path: str, id: str) -> None:
    ...
    # plt.show()
    plt.savefig(
        base_path + id + " ground_truth_plot.png", dpi=300, bbox_inches="tight"
